...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    out = result.stdout
    # AGENTS.md directive #12: CLI banner/version assertions must use THE JANITOR v.
    for field in ("THE JANITOR v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    # AGENTS.md directive #12: CLI banner/version assertions must use THE JANITOR v.
    assert "THE JANITOR v" in result.stdout
